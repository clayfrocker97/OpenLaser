<script lang="ts">
  ...
  const priceText = (gas: GasKind): string => {
    ...
    if (source.kind === 'compressor') return source.cost_per_hour > 0 ? `${money(source.cost_per_hour, costs.currency)} per hour open` : 'No running cost yet';
...
<div class="setting-group gas-costs"><h3>Gas costs</h3>
  <p class="intro">Prices turn each job's gas time into money on the Setup and Run screens. Flow is estimated from the recipe's nozzle and pressure unless you enter one; calibrate it with a 60-second test.</p>
  {#if problem}<p class="warn-text intro">{problem}</p>{/if}
  <div class="setting"><div class="lbl">Currency symbol<small>Shown before every amount</small></div><button class="val" onclick={() => osk.text('Currency symbol', costs.currency, v => { const t = v.trim(); if (t && [...t].length <= 4) change(c => { c.currency = t; }); else ui.say('Use 1 to 4 characters.', true); })}>{costs.currency}</button></div>
    ...
    <section class="supply" style="--gas:{GAS_COLORS[gas]}">
      ...
      <div class="setting"><div class="lbl">Source</div>
        <div class="seg" role="group" aria-label="{GAS_NAMES[gas]} source">
          {#if gas === 'air'}<button class:on={supply.source.kind === 'compressor'} aria-pressed={supply.source.kind === 'compressor'} onclick={() => setSource(gas, 'compressor')}>Compressor</button>{/if}
          <button class:on={supply.source.kind === 'refill'} aria-pressed={supply.source.kind === 'refill'} onclick={() => setSource(gas, 'refill')}>{gas === 'air' ? 'Cylinder' : 'Refills'}</button>
          {#if gas !== 'air'}<button class:on={supply.source.kind === 'bulk'} aria-pressed={supply.source.kind === 'bulk'} onclick={() => setSource(gas, 'bulk')}>Bulk per m³</button>{/if}
        </div>
      </div>
      {#if supply.source.kind === 'refill'}
        {@const source = supply.source}
        <div class="setting"><div class="lbl">Price per refill</div><button class="val" data-numpad onclick={() => number(`${GAS_NAMES[gas]} price per refill`, source.price, costs.currency, v => change(c => { c[gas].source = { ...source, price: v }; }))}>{money(source.price, costs.currency)}</button></div>
        <div class="setting"><div class="lbl">Cylinder volume<small>Gas content at standard conditions</small></div><button class="val" data-numpad onclick={() => number(`${GAS_NAMES[gas]} cylinder volume`, source.volume, 'm³', v => { if (v > 0) change(c => { c[gas].source = { ...source, volume: v }; }); })}>{source.volume} m³</button></div>
      {:else if supply.source.kind === 'bulk'}
        {@const source = supply.source}
        <div class="setting"><div class="lbl">Price per m³</div><button class="val" data-numpad onclick={() => number(`${GAS_NAMES[gas]} price per m³`, source.price_per_m3, costs.currency, v => change(c => { c[gas].source = { ...source, price_per_m3: v }; }))}>{money(source.price_per_m3, costs.currency)}</button></div>
      {:else}
        {@const source = supply.source}
        <div class="setting"><div class="lbl">Running cost<small>Charged per hour the valve is open</small></div><button class="val" data-numpad onclick={() => number('Compressor running cost per hour', source.cost_per_hour, `${costs.currency}/h`, v => change(c => { c[gas].source = { ...source, cost_per_hour: v }; }))}>{money(source.cost_per_hour, costs.currency)}/h</button></div>
      {/if}
      <div class="setting"><div class="lbl">Flow<small>L/min</small></div>
        <div class="seg" role="group" aria-label="{GAS_NAMES[gas]} flow">
          <button class:on={supply.flow.kind === 'estimated'} aria-pressed={supply.flow.kind === 'estimated'} onclick={() => change(c => { c[gas].flow = { kind: 'estimated' }; })}>Nozzle estimate</button>
          <button class:on={supply.flow.kind === 'manual'} aria-pressed={supply.flow.kind === 'manual'} onclick={() => change(c => { if (c[gas].flow.kind !== 'manual') c[gas].flow = { kind: 'manual', rate: 100 }; })}>Manual</button>
        </div>
      </div>
