...
<div class="setting-group manual-tests"><h3>Manual tests</h3>
  ...
  <div class="test-fields">
    ...
    <button class="btn btn-primary" disabled={!doc.readiness.jog.ok} onclick={() => run(() => api.machine('pulse', { pulse: { duration_ms: pulseDuration, power: pulsePower } }))}>Pulse laser</button>
  </div>
  ...
  <div class="test-fields gas-fields">
    ...
    <button class="btn btn-soft" disabled={!doc.readiness.outputs.ok || !doc.bindings?.outputs.gas[gas]} onclick={() => run(() => api.machine('gas-test', { gas_test: { selector: gas, pressure, duration_ms: gasDuration } }))}>Test gas</button>
  </div>
</div>
...
<style>
  ...
  .gas-fields > .btn { grid-column: 1 / -1; }
