  ...
  {#if data}
    <div class="cal-body">
      ...
      <div class="step-content">
        ...
        {:else}
          <span class="eyebrow">3 · Apply</span><h3>{data.enabled ? 'Correction is enabled' : 'Ready for future DXFs'}</h3><p>Applies to new {laserLabel(mode)} DXF jobs.</p><dl><div><dt>X measurements</dt><dd>{range('x')}</dd></div><div><dt>Y measurements</dt><dd>{range('y')}</dd></div><div><dt>Squares measured</dt><dd>{count} / 9</dd></div></dl><button class="btn btn-primary lg" disabled={busy || count !== 9} onclick={() => run(apply)}>Save &amp; apply to new DXFs</button>{#if data.active}<button class="text-action" disabled={busy} onclick={() => run(enable)}>{data.enabled ? 'Pause correction for new jobs' : 'Enable saved profile'}</button>{/if}
        {/if}
...
<style>
  ...
  nav { display:flex; gap:8px; margin-bottom:30px; } nav button { flex:1; display:flex; align-items:center; gap:12px; padding:12px 18px; min-height:58px; border:1px solid var(--line); border-radius:10px; color:var(--ink-3); background:var(--panel-2); text-align:left; font-size:15px; cursor:pointer; }
  ...
  .bed strong { font-size:25px; } .bed span { font-size:12px; } .bed small { font-size:10px; color:var(--ink-3); } .bed button.measured { border-color:var(--ok); } .bed button.chosen { outline:3px solid var(--accent); outline-offset:3px; }
  .bed-heading { display:flex; justify-content:space-between; gap:15px; color:var(--ink-3); font-size:11px; padding:10px 0; }
  .progress { color:var(--ink-3); font-size:12px; margin:8px 0; } .eyebrow { text-transform:uppercase; letter-spacing:.12em; font-size:11px; color:var(--accent); font-weight:700; }
  .step-content { padding-top:18px; } h3 { font-size:24px; line-height:1.2; margin:12px 0 18px; color:var(--ink); text-transform:none; letter-spacing:normal; } .step-content p { color:var(--ink-3); font-size:14px; line-height:1.7; margin:0 0 18px; }
  .step-content .btn { width:100%; min-height:56px; margin-top:10px; } .text-action { min-height:50px; width:100%; border:0; background:none; color:var(--ink-2); font-size:13px; cursor:pointer; margin-top:8px; }
  .measurement { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin:24px 0 14px; } .measurement button { min-height:104px; text-align:left; padding:16px; border:1px solid var(--line); border-radius:10px; background:var(--panel-2); color:var(--ink); cursor:pointer; } .measurement span { display:block; font-size:12px; color:var(--ink-3); margin-bottom:12px; } .measurement strong { font-size:19px; }
  .step-content dl { margin:25px 0; } dl div { display:flex; justify-content:space-between; gap:15px; padding:13px 0; border-bottom:1px solid var(--line); font-size:13px; } dt { color:var(--ink-3); } dd { margin:0; } .error { color:var(--warn); } .step-content .saved { color:var(--ok); margin-top:16px; } header .seg button { min-height:48px; }
  @media(max-width:950px) { .cal-body { gap:22px; } .bed { gap:12px; padding:14px; } .bed small { display:none; } }
  @media(max-width:720px) { .cal-body { grid-template-columns:1fr; } header { align-items:flex-start; } nav button { padding:10px; gap:8px; font-size:13px; } }
  @media(max-height:850px) { header { margin-bottom:12px; } nav { margin-bottom:14px; } nav button { min-height:50px; padding-block:8px; } .bed { gap:10px; padding:12px; } .bed button { min-height:80px; gap:4px; padding:7px; } .bed strong { font-size:22px; } .bed-heading { padding:8px 0; } .step-content { padding-top:8px; } h3 { font-size:22px; margin:10px 0 12px; } .step-content p { font-size:13px; margin-bottom:12px; } .measurement { margin:14px 0; } }
</style>
