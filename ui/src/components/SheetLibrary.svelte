<script lang="ts">
  ...
  const jobs = $derived((server.doc?.library.jobs ?? [])
    .filter(j => !search.trim() || `${j.name} ${j.recipe.name}`.toLowerCase().includes(search.toLowerCase()))
    .slice().reverse().slice(0, 60));
  function compatible(sheet: SheetView): boolean {
    return !recipe || (recipe.laser === sheet.mode
  ...
  function sheetStatus(sheet: SheetView): string {
    ...
    if (sheet.used) return 'Used by a later cut';
    if (sheet.state === 'remnant') return 'Ready to nest';
    if (sheet.state === 'completed') return 'Ready to inspect';
    return 'Unfinished cut';
  }
  async function load(more = false): Promise<void> {
    busy = true; error = '';
    try {
      const page = await api.sheets({ remnants, before: more ? next ?? undefined : undefined });
  ...
  function closeAdding(): void { if (!busy) adding = false; }
  ...
  async function report(id: string): Promise<void> {
    busy = true; error = '';
    try {
      const result = await api.reportCutSheet(id);
      adding = false;
      inspected = result.id;
      remnants = false;
      await load();
...
<div class="sheet-library">
  ...
  <div class="sheets-grid">
    {#each items as sheet}
      <button class="sheet-card" disabled={busy || (!!onchoose && !compatible(sheet))} onclick={() => choose(sheet)}>
        <div class="shape">
          <SheetShape
            outline={sheet.outline}
            cutouts={sheet.cutouts}
            label={`${sheet.name}, ${plural(sheet.cutouts.length, 'cut area')}`}
          />
        </div>
        <div class="card-info">
...
{#if adding}
  <Modal title="Mark a saved job as already cut" onclose={closeAdding}>
    ...
    <div class="job-choices">
      {#each jobs as job}
        <button disabled={busy} onclick={() => report(job.id)}>
          <strong>{job.name}</strong>
          <span>{job.recipe.name} · {quantity(job.recipe.thickness_mm, 'mm')} · {laserLabel(job.recipe.laser)}</span>
        </button>
      {/each}
    </div>
...
<style>
  ...
  .card-info { padding:16px; display:grid; gap:7px; }
  .card-info strong { font-size:var(--t-base); overflow-wrap:anywhere; }
  .card-info span { font-size:var(--t-sm); color:var(--ink-2); }
  .card-info small { font-size:var(--t-sm); color:var(--ink-3); }
  em { font-style:normal; font-size:var(--t-sm); color:var(--accent); margin-top:3px; }
  .intro, .empty p { color:var(--ink-3); font-size:var(--t-sm); line-height:1.65; }
  .empty { max-width:420px; padding:65px 20px; margin:auto; text-align:center; }
  .empty strong { font-size:var(--t-lg); }
  .more { display:block; min-height:48px; margin:22px auto 0; }
  .error { color:var(--warn); }
  .job-search { width:100%; min-height:48px; padding:12px; border:1px solid var(--line); border-radius:8px; background:var(--panel-2); color:var(--ink); margin:12px 0; }
  .job-choices { display:grid; gap:8px; max-height:50vh; overflow:auto; }
  .job-choices button { min-height:72px; padding:16px; text-align:left; border:1px solid var(--line); border-radius:9px; background:var(--panel-2); color:var(--ink); cursor:pointer; }
  .job-choices span { display:block; font-size:var(--t-sm); margin-top:7px; color:var(--ink-3); }
</style>
