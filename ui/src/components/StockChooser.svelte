...
<style>
  ...
  .new-stock { display:grid; grid-template-columns:1fr 1fr; align-items:center; gap:36px; padding:20px 10px; } .sheet-icon { display:grid; place-items:center; min-height:240px; margin:15px; background:var(--accent-soft); border:2px solid var(--line); border-radius:5px; color:var(--ink-3); font-size:15px; }
  h3 { font-size:22px; color:var(--ink); text-transform:none; letter-spacing:normal; margin:0 0 16px; } p { font-size:14px; line-height:1.7; color:var(--ink-3); } .dimensions { display:grid; grid-template-columns:1fr 1fr; gap:12px; margin:24px 0; } .dimensions button { min-height:86px; padding:14px; text-align:left; border:1px solid var(--line); border-radius:10px; background:var(--panel-2); color:var(--ink); cursor:pointer; } .dimensions span { display:block; font-size:12px; margin-bottom:10px; color:var(--ink-3); } .dimensions strong { font-size:18px; } .btn.lg { min-height:56px; } .drawing-stock { padding:40px; max-width:570px; margin:auto; text-align:center; } .error { color:var(--warn); }
