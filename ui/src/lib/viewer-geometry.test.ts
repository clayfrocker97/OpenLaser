...
describe('viewer bounds and cached paths', () => {
  ...
  it('grows halos inside a shape until they meet, and keeps them fixed outside', () => {
    ...
    const index = new BoundsIndex(boundsOfShapes(shapes));
    const pick = (at: [number, number]) => (pickLine(shapes, index, at, 2)?.contour as { sources: number[] } | undefined)?.sources;
    expect(pick([0.5, 0.5])).toEqual([1]);
    expect(pick([15, 0])).toEqual([1]);
    expect(pick([30, 0])).toEqual([2]);
    expect(pick([41, 0])).toEqual([2]);
    expect(pick([60, 0])).toBeUndefined();
  });
});
