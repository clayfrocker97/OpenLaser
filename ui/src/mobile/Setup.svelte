<script lang="ts">
  ...
  function save(): void {
    if (!draft) return;
    osk.text(draft.sheets?.pages.some(p => !p.job) ? 'Folder for numbered sheets' : 'Job name',job?.name ?? part?.name ?? 'job',name => {
      if (!name.trim()) return;
      void perform(async () => {
  ...
  <div class="phone-run-scroll">
    ...
    {#if draft}
      <section class="phone-job-card">
        <div class="phone-job-heading"><div><h2>{job?.name ?? part?.name ?? 'Current job'}</h2></div></div>
        <Preview {outline} />
