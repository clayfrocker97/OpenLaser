<script lang="ts">
  ...
  const recipes = $derived(doc.library.recipes
    .filter(r => recipeLabel(r).toLocaleLowerCase().includes(search.toLocaleLowerCase()))
    .sort((a,b) => Number(b.laser === doc.mode) - Number(a.laser === doc.mode)
      || Number(b.favourite) - Number(a.favourite)
      || a.name.localeCompare(b.name)));
  const materialName = $derived(draft?.recipe ? recipeLabel(draft.recipe) : 'Choose a material');
  ...
  function review(dryRun:boolean): void {
    void perform(async () => {
      if (draft?.dry_run !== dryRun) await api.compile(dryRun);
      ui.tab = 'run';
    });
  }
  ...
  function openTool(tool: FeatureId | 'copy' | 'nest'): void {
    tools = false;
    edit(tool);
  }
</script>
...
<div class="phone-run phone-setup-home">
  <div class="phone-run-scroll">
    ...
    {#if draft}
      <section class="phone-job-card">
        ...
        <Preview {outline} />
        <div class="phone-job-spec">
          <span>{size(draft.preview?.bounds ?? part?.bounds ?? null)}</span>
          <button class="phone-text-action" disabled={blocked} onclick={() => edit('layout')}>Edit layout</button>
        </div>
      </section>
      ...
      <div class="phone-action-list phone-setup-options">
        <button disabled={blocked} onclick={() => adding = true}>
          <span>Parts<small>{partsHint}</small></span><i class="ic ic-plus"></i>
        </button>
        <button disabled={blocked} onclick={() => texting = true}>
          <span>Add text<small>Letters welded into cutting outlines</small></span><i class="ic ic-plus"></i>
        </button>
        <button disabled={blocked} onclick={() => tools = true}>
    ...
    {:else}
      <div class="phone-empty">
        ...
        <h2>No part open</h2>
        <p>Choose a part or saved job to set up, or start from text.</p>
        <button class="phone-primary" onclick={() => ui.tab = 'parts'}>Go to Parts</button>
        <button class="phone-text-action" onclick={() => texting = true}>Add text</button>
      </div>
    {/if}
  </div>
  {#if draft}<div class="phone-run-footer">
    ...
    <button class="phone-primary" disabled={blocked || !doc.readiness.compile.ok} onclick={() => review(false)}>
      {busy ? 'Preparing…' : 'Go to Run'}<i class="ic ic-arrow-right"></i>
    </button>
    {#if !doc.readiness.compile.ok}<p class="phone-readiness">{plain(doc.readiness.compile.reason).text}</p>{/if}
  </div>{/if}
</div>

{#if material}<Modal title="Material" onclose={() => material = false}>
  <div class="phone-material-picker">
    <input type="search" aria-label="Search materials" placeholder="Search materials" bind:value={search} />
...
{#if tools}<Modal title="Machining" onclose={() => tools = false}>
  <div class="phone-action-list">
    {#each TOOLS as tool}
      <button onclick={() => openTool(tool.id as FeatureId)}>
        <span>{tool.name}<small>{draft ? stateOf(draft.features,tool.id) : ''}</small></span><i class="ic ic-arrow-right"></i>
      </button>
    {/each}
    <button onclick={() => openTool('nest')}>Nest parts<i class="ic ic-arrow-right"></i></button>
    <button onclick={() => openTool('copy')}>Copy machining from job<i class="ic ic-arrow-right"></i></button>
  </div>
</Modal>{/if}
