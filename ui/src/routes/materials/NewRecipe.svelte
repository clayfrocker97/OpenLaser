<script lang="ts">
  ...
  let draft = $state({ laser: 'fiber' as LaserMode, name: '', thickness_mm: 1, gas: null as number | null, values: null as Values | null });
  let search = $state('');
  let category = $state('');
  ...
  const matching = $derived(cards.filter((c) => (!category || c.category === category) && (!search.trim() || c.name.toLowerCase().includes(search.trim().toLowerCase()))));
  ...
  async function add(): Promise<void> {
    ...
    try {
      const { id } = await api.addRecipe({ name, laser: draft.laser, thickness_mm: draft.thickness_mm, values: draft.values, gas: draft.laser === 'fiber' ? draft.gas : null });
...
<section class="panel main">
  <div class="panel-head">
    ...
    <div class="actions">
      <div class="seg">{#each [['fiber', 'Fiber'], ['co2', 'CO₂']] as [k, label] (k)}<button class:on={draft.laser === k} onclick={() => setLaser(k as LaserMode)}>{label}</button>{/each}</div>
      <button class="btn btn-ghost icon-only" onclick={onclose} aria-label="Close"><i class="ic ic-x"></i></button>
    </div>
  </div>
  <div class="toolbar">
    {#if categories.length > 1}<div class="chips cats"><button class="chip" class:on={!category} onclick={() => (category = '')}>All</button>{#each categories as c (c)}<button class="chip" class:on={category === c} onclick={() => (category = category === c ? '' : c)}>{c}</button>{/each}</div>{/if}
  ...
  <div class="grid pick-grid" bind:this={grid}>
    {#each paged as c (c?.id ?? 'custom')}
      {#if c === null}
        <button class="pick" class:on={custom} onclick={() => osk.text('Material', custom ? draft.name : '', (v) => { draft.name = v.trim(); })}><span class="art blank">Aa</span><span>{custom ? draft.name : 'Custom name…'}</span></button>
      {:else}
        <button class="pick" class:on={draft.name === c.name} onclick={() => (draft.name = c.name)}><img class="art" src={cardUrl(c.id)} alt="" loading="lazy"><span>{c.name}</span></button>
      {/if}
    {/each}
  </div>
  <div class="new-foot">
    <div class="field"><h3>Thickness</h3><button class="btn btn-ghost" data-numpad onclick={() => osk.number('Thickness', draft.thickness_mm, 'mm', (v) => { if (v >= 0) draft.thickness_mm = v; })}>{mm(draft.thickness_mm)}</button></div>
    {#if draft.laser === 'fiber'}
      <div class="field"><h3>Assist gas</h3><div class="chips">{#each gases as selector (selector)}<button class="chip" class:on={draft.gas === selector} onclick={() => (draft.gas = selector)}>{GAS[selector] ?? `Selection ${selector}`}</button>{/each}</div></div>
    {/if}
    <div class="field grow">
      <h3>Start from</h3>
      <div class="from-row">
        {#each sources as source (source.label)}
          <button class="from" class:on={same(draft.values, source.values)} onclick={() => choose(source)}><span>{source.label}</span><small>{source.meta}</small></button>
        {:else}
