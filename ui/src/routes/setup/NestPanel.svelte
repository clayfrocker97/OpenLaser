<script lang="ts">
  ...
  const stockName = $derived(
    stock?.kind === 'remnant' ? stock.name
    : stock?.kind === 'outline' ? 'Drawing outline'
    : stock?.kind === 'rectangle'
      ? `${distance(stock.bounds.max.x - stock.bounds.min.x)} × ${distance(stock.bounds.max.y - stock.bounds.min.y)} ${unitLabel('mm')}`
    : 'Choose a sheet');
  const stockEyebrow = $derived(stock?.kind === 'remnant' ? 'Remnant stock' : 'Stock');
  const stockDetail = $derived(
    stock?.kind === 'remnant' ? `${draft.stock_cutouts.length} existing cut areas kept clear`
    : stock?.kind === 'outline' ? 'Reference boundary · excluded from cutting'
    : 'New sheets of the same size');
  ...
  const overflowNote = $derived(
    stock?.kind === 'remnant' ? 'Overflow uses fresh rectangular sheets; this remnant is used once.' : 'Part holes stay reserved.');
  ...
  async function start(): Promise<void> {
    ...
    try {
      result = await api.nest(
        { contours: effectiveSelection, quantity, settings: $state.snapshot(settings), seconds }, draft.revision);
  ...
  <div class="preview-pages" role="group" aria-label="Nesting preview sheets">
    {#each result.sheets as sheet, i}
      <button class:on={page === i} aria-pressed={page === i} disabled={busy} onclick={() => show(i)}>
        ...
        {#if sheet.fresh}<small>Fresh sheet</small>{/if}
      </button>
    {/each}
  </div>
...
{:else if result?.running}
  <div class="searching" role="status">
    ...
    <p>{result.placed} of {result.total} parts placed</p>
    <progress value={result.placed} max={result.total}></progress>
    <p>{liveNote}</p>
...
{:else}
  ...
  <fieldset disabled={busy}>
    <button class="stock-card" onclick={() => stockOpen = true}>
      <span class="eyebrow">{stockEyebrow}</span>
      <strong>{stockName}</strong>
      <span>{stockDetail} <b>Change ›</b></span>
    </button>
    {#if ui.nestPicking}
      <p class="pick-hint">Tap the closed stock outline on the drawing.<button
    ...
    <div class="distances">
      ...
      <button class="value" onclick={() => number('Stock edge margin', settings.margin, 'mm', v => settings.margin = v)}>
        <span>Edge margin</span>
        <strong>{measure(settings.margin, 'mm')}</strong>
      </button>
    </div>
    {#if stock?.kind === 'remnant'}
      <button
    ...
    <p class="nest-note">Lead and kerf clearance is added. {overflowNote}</p>
...
<style>
  ...
  .stock-card { width:100%; text-align:left; padding:17px; background:var(--panel-2); border:1px solid var(--line); border-radius:11px; color:var(--ink); cursor:pointer; display:grid; gap:9px; }
  .stock-card > strong { font-size:var(--t-lg); }
  .stock-card > span:last-child { color:var(--ink-3); font-size:var(--t-sm); line-height:1.7; }
  .stock-card b { display:block; color:var(--accent); margin-top:2px; }
  .eyebrow { font-size:var(--t-sm); text-transform:uppercase; letter-spacing:.1em; color:var(--ink-3); }
